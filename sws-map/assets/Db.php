...
class Db {
	...
	public function connect() {
		...
		if (isset(self::$connection)) { //echo "ALREAY ACTIVE!";
		}
		// Try and connect to the database
		if(!isset(self::$connection)) {
			// Load configuration as an array. Use the actual location of your configuration file
			// Put the configuration file outside of the document root
			$path='/etc/ccharacter/'.$which.'_config.ini'; // echo $path;
			$config = parse_ini_file($path); //print_r($config);
			// $config = parse_ini_file('configs/'.$which.'_config.ini'); 
			
			self::$connection = new mysqli('bismark',$config['username'],$config['password'],$config['dbname']);
